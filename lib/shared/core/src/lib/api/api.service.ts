...
export class ApiService {
    ...
    post<T>(url: string, data: Record<string, unknown> = {}) {
        return this._http.post<T>(this._createApiUrl(url, data), data).pipe(timeout(this._config.timeout));
    }

    put<T>(url: string, data: Record<string, unknown> = {}) {
        return this._http.put<T>(this._createApiUrl(url, data), data).pipe(timeout(this._config.timeout));
    }

    patch<T>(url: string, data: Record<string, unknown> = {}) {
        return this._http.patch<T>(this._createApiUrl(url, data), data).pipe(timeout(this._config.timeout));
    }

    delete<T>(url: string, data: Record<string, unknown> = {}) {
        return this._http.delete<T>(this._createApiUrl(url, data)).pipe(timeout(this._config.timeout));
    }
}
